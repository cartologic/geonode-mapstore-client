...
import { hashLocationToHref, getFilterById } from "@js/utils/GNSearchUtils";
...
const ConnectedLanguageSelector = connect(
    createSelector([currentLocaleSelector], (value) => ({
        value,
    })),
    {
...
const CardGridWithMessageId = ({ query, user, isFirstRequest, ...props }) => {
    ...
    const hasFilter =
        Object.keys(query || {}).filter((key) => key !== "sort").length > 0;
    const isLoggedIn = !!user;
    const messageId =
        !hasResources && !isFirstRequest && !props.loading
...
const suggestionsRequestTypes = {
    resourceTypes: {
        ...
        loadOptions: (q, params) =>
            getResourceTypes({ ...params, q }, "filter{resource_type.in}")
                .then((results) => ({
                    options: results.map(({ selectOption }) => selectOption),
                }))
                .catch(() => null),
    },
    categories: {
        ...
        loadOptions: (q, params) =>
            getCategories({ ...params, q }, "filter{category.identifier.in}")
                .then((results) => ({
                    options: results.map(({ selectOption }) => selectOption),
                }))
                .catch(() => null),
    ...
    keywords: {
        ...
        loadOptions: (q, params) =>
            getKeywords({ ...params, q }, "filter{keywords.slug.in}")
                .then((results) => ({
                    options: results.map(({ selectOption }) => selectOption),
                }))
                .catch(() => null),
    ...
    regions: {
        ...
        loadOptions: (q, params) =>
            getRegions({ ...params, q }, "filter{regions.name.in}")
                .then((results) => ({
                    options: results.map(({ selectOption }) => selectOption),
                }))
                .catch(() => null),
    ...
    owners: {
        ...
        loadOptions: (q, params) =>
            getOwners({ ...params, q }, "filter{owner.username.in}")
                .then((results) => ({
                    options: results.map(({ selectOption }) => selectOption),
                }))
                .catch(() => null),
...
function Home({
    ...
    function handleClear() {
        const { query } = url.parse(location.search, true);
        const newParams = Object.keys(query).reduce(
            (acc, key) =>
    ...
    const queryFilters = Object.keys(query).reduce(
        (acc, key) =>
            key.indexOf("filter") === 0
                ? [
                      ...acc,
                      ...castArray(query[key]).map((value) => ({ key, value })),
                  ]
                : acc,
        []
    );
    ...
    useEffect(() => {
        const suggestionsRequestTypesArray = Object.keys(
            suggestionsRequestTypes
        ).map((key) => suggestionsRequestTypes[key]);
        const queryKeys = Object.keys(state.current.query);
        let updateRequests = [];
        queryKeys.forEach((queryKey) => {
            const suggestionRequest = suggestionsRequestTypesArray.find(
                ({ filterKey }) => queryKey === filterKey
            );
            if (suggestionRequest) {
                const filtersToUpdate = castArray(
                    state.current.query[queryKey]
                ).filter((value) => !getFilterById(queryKey, value));
                if (filtersToUpdate?.length > 0) {
                    const request = suggestionRequest.loadOptions.bind(
                        null,
                        "",
                        {
                            idIn: filtersToUpdate,
                        }
                    );
        ...
        Promise.all(updateRequests.map((request) => request())).then(() => {
    ...
    return (
        <div className={`gn-home gn-theme-${theme?.variant || "light"}`}>
            <BrandNavbar
                ref={brandNavbarNode}
                logo={castArray(navbar?.logo || []).map((logo) => ({
                    ...logo,
                    ...logo[pageSize],
                }))}
            ...
            {!hideHero && (
                ...
                >
                    <div ref={inViewRef}>{isHeroVisible && search}</div>
                </Hero>
            )}
            <MenuIndex
                ref={menuIndexNode}
                    ...
                    !isHeroVisible
                        ? {
                              ...
                              minHeight: `calc(100vh - ${
                                  dimensions.brandNavbarHeight +
                                  dimensions.menuIndexNodeHeight +
                                  dimensions.footerNodeHeight
                              }px )`,
