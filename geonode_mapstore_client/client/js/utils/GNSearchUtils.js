...
export const getFilterLabelById = (filterKey = "", id) =>
    filters?.[filterKey + id]?.selectOption?.label;
...
export const hashLocationToHref = ({
    ...
    const newQuery = query
        ? replaceQuery
            ...
            : Object.keys(query).reduce((acc, key) => {
                  ...
                  const currentQueryValues = castArray(acc[key]).filter(
                      (val) => val
                  );
                  const queryValue =
                      currentQueryValues.indexOf(value) === -1
    ...
    return `#${url.format({
        ...loc,
        ...(pathname && { pathname }),
        query: Object.keys(newQuery).reduce(
            (acc, newQueryKey) =>
...
export const getResourceTypesInfo = () => ({
    layer: {
        icon: "layer-group",
        formatEmbedUrl: (embedUrl) =>
            updateUrlQueryParameter(embedUrl, {
